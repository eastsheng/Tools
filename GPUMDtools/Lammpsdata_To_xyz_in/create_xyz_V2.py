...
class CreateXYZ(object):
	...
	def writexyz_V1(self,M1=0,M2=0,M3=0):
		...
		# print(atom_m.shape)
		self.atom_txyzm = np.hstack((atom_txyz,atom_m))
		return

	def ExchangeAtomeType(self,):
		at = self.atom_txyzm
		lenat = len(at)
		# print(at[:,0])
		for i in range(lenat):
			if at[i,0] == 0:
				...
				at[i,0] = 0
		# print(at[:,0])
		atom_t = at[:,0].reshape(lenat,1)
		atom_xyzm = at[:,1:]
		print(atom_xyzm.shape)
		self.atom_txyzm = np.hstack((atom_t,atom_xyzm))
		return


	def savefile(self,savedata):

		np.savetxt(savedata,self.atom_txyzm,fmt='%d %.9f %.9f %.9f %.9f',
			header=self.line01)
...
# exchange the types of atoms, for example, from (S Mo = 0 1) to (Mo S = 0 1)
ETA = True
####################Variables####################
...
xyz.writexyz_V1(M1,M2)
if ETA == True:
	xyz.ExchangeAtomeType()
else:
	pass
xyz.savefile(savedata)
print('+--------------------------+\n')