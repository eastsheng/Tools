...
class Dispersion(object):
	...
	def plot_energy(self,figname_energy):
		...
		fig.colorbar(line)
		# 格式
		# plt.title('$\mathregular{MoS_2}$/$\mathregular{MoSe^{209}}_\mathregular{2}$', size=36,fontweight='bold')
		...
		# plt.xticks([self.r_xmin,self.r_xmax], ['Γ','X'],size=36,fontweight='bold')
		plt.xticks([0,0.5,1.0,1.5,2], ['Γ','X','M','Y','Γ'],size=26,fontweight='bold')
		plt.yticks(size=26,fontweight='bold')
		# 范围
		plt.xlim(self.r_xmin, self.r_xmax)
		plt.ylim(self.r_ymin, self.r_ymax)
		# 保存
...
disper_before_ener = path+'phonon_sorted_energy'+str(iso)+'_all.dat'
# 要保存的文件
# disper_after = 'p_sorted'
disper_after_ener = path+'p_sorted_energy_all'+str(iso)

# 保存的图片
disperfig = path+'phonon_disp'+str(iso)+'_all.tiff'
disperEnergyfig = path+'phonon_dispEnergy'+str(iso)+'_all.tiff'
