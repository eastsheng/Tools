...
class Dispersion(object):
	...
	def plot_energy(self,figname_energy):
		...
		# 格式
		# plt.title('Dipersion', size=26)
		ax.set_xlabel('Wave vector',fontsize=48,fontweight='bold')
		ax.set_ylabel('Frequency (THz)',fontsize=48,fontweight='bold')
...
disper_before_ener = path+'phonon_sorted_energy'+str(iso)+'.dat'
# 要保存的文件
# disper_after = 'p_sorted'
disper_after_ener = path+'p_sorted_energy'+str(iso)

# 保存的图片
disperfig = path+'phonon_disp'+str(iso)+'.tiff'
disperEnergyfig = path+'phonon_dispEnergy'+str(iso)+'.tiff'
